...
const appList_Array = [
...

  {
    ...
    require: "Từ 18 tuổi trở lên",
    price: "20K tiền công + 50K Từ App SeABank",
...
oneApp.insertAdjacentHTML("beforeEnd", `
...
btn_success.forEach((btn) => {
  btn.addEventListener("click", () => {
    const input_value = btn.parentElement.parentElement.querySelector("input").value.trim().toLocaleLowerCase();
    if(input_value.length < 4){
      // alert("Username cần phải có từ 4 ký tự trở lên");
      alert_message.textContent = "Username cần phải có từ 4 ký tự trở lên! VD ngoc1234";
    ...
    if (app) {
      if(app.source === "accesstrade"){
        aLinkDownload.href = app.linkDownload_first + `utm_source=${input_value}&` + app.linkDownload_last;
...
inputList.forEach(input => {
  input.addEventListener("keypress", (e) => {
    let key = e.keyCode;
    if(key === 32 || e.code === "Space" || key === 160 || key === 5760 || key === 8192 || key === 8192 || key === 8194 || key === 8195 || key === 8196 || key === 8197 || key === 8198 || key === 8199 || key === 8200 || key === 8201 || key === 8202 || key === 8232 || key === 8233 || key === 8239 || key === 8287 || key === 12288){
      e.preventDefault();
    }
  })
})
